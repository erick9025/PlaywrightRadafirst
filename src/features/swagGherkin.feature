Feature: SWAG pages
  ...
  Scenario Outline: DDT: Successful login with valid credentials to Swag Sauce
    Given I am on the Swag login page
    When I login to Swag with username "standard_user" and password "secret_sauce"
    Then I should be redirected to the Swag dashboard
  ...
  Scenario Outline: DDT Two Columns: Successful login with valid credentials to Swag Sauce
    Given I am on the Swag login page
    When I login to Swag with username "standard_user" and password "secret_sauce"
    Then I should be redirected to the Swag dashboard
    When I add "<product>" product to the cart
    ...
    Examples:
      | product                       | message |
      | Sauce Labs Backpack           | uno     |
      | Sauce Labs Bike Light         | dos     |
      | Sauce Labs Bolt T-Shirt       | tres    |
      | Sauce Labs Fleece Jacket      | cuatro  |
      | Sauce Labs Onesie             | cinco   |
      | Test.allTheThings() T-Shirt   | seis    |