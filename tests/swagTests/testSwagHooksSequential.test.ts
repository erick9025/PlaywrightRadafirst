...
test.describe('Tests for Swag pages SEQUENTIAL/SERIAL (Simulated with 1 thread)', () => {
    ...
    test("[1001] Pt: 1 - Swag Add products and go to cart with hooks", async ({ PagesSauceLabs }) => {         
        await PagesSauceLabs.SwagLoginPage.login();
        didPart1Pass = true;
    });

    test("[1002] Pt: 2 - Swag Add products and go to cart with hooks", async ({ PagesSauceLabs }) => {  
    ...
    test("[1003] Pt: 3 - Swag Add products and go to cart with hooks", async ({ PagesSauceLabs }) => {       
        if(!didPart2Pass) {
    ...
    test("[1004] Pt: 4 - Swag Add products and go to cart with hooks", async ({ PagesSauceLabs }) => {  
        if(!didPart3Pass) {
...
test.describe.serial('Tests for Swag pages SEQUENTIAL/SERIAL (Real sequential)', () => {
    test.use({ createContextBeforeEachTest: false }); // Set to false to create a single context for all tests (faster but less isolated)
    ...
    test("[1001] bPt: 1 - Swag Add products and go to cart with hooks", async ({ PagesSauceLabs }) => {         
        await PagesSauceLabs.SwagLoginPage.login();
    });

    test("[1002] bPt: 2 - Swag Add products and go to cart with hooks", async ({ PagesSauceLabs }) => {  
        await PagesSauceLabs.SwagProductsPage.addProductToCart("Sauce Labs Backpack");
        await PagesSauceLabs.SwagProductsPage.addProductToCart("Sauce Labs Fleece Jacket");                
    });

    test("[1003] bPt: 3 - Swag Add products and go to cart with hooks", async ({ PagesSauceLabs }) => {       
        await PagesSauceLabs.SwagProductsPage.sortProducts(ProductSortingOptions.NameAscending);
        await PagesSauceLabs.SwagProductsPage.printTotalAddedSoFar();
    ...
    test("[1004] bPt: 4 - Swag Add products and go to cart with hooks", async ({ PagesSauceLabs }) => {  
        await PagesSauceLabs.SwagCartPage.goToCart();        
        await PagesSauceLabs.SwagCartPage.verifyCartTotalIsCorrect();
    });
});